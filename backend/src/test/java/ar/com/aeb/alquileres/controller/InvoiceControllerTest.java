...
class InvoiceControllerTest extends BaseControllerTest {
    ...
    private Long buildPaymentAndGetId(BigDecimal amount) throws Exception {
        ...
        String response = mockMvc.perform(multipart("/api/v1/properties/" + property.getId() + "/payments").param("amount", amount.toPlainString()).param("paymentMethod", "BANK_TRANSFER").param("paymentDate", LocalDate.now().toString()).param("selectedPeriods", "2026-05")).andReturn().getResponse().getContentAsString();
